// 🔐 OPENROUTER API KEY 
const API_KEY = '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';
...
// ===== SUGGESTION CHIPS =====
// Add event listeners to initial chips
document.querySelectorAll('.chip').forEach(chip => {
    chip.addEventListener('click', () => {
        const text = chip.textContent.replace(/^[^\s]+\s/, ''); // Remove emoji
...
function addFilePreview(file) {
    ...
    removeBtn.innerHTML = '<i class="fa-solid fa-times"></i>'; // Use Icon
    removeBtn.onclick = () => removeFile(file.name);
    preview.appendChild(removeBtn);

    filePreviewContainer.appendChild(preview);
    filePreviewContainer.classList.add('active'); // Ensure visible
}
...
async function addMessageToUI(role, content, files = []) {
    ...
    // Use FontAwesome icons for avatars
    ...
    // Handle Markdown-like basic formatting (optional, simple replacement for now)
    // For a real app, use a library like marked.js
